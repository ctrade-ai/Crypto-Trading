...
const { ORDER_STATUS, TRANSACTION_ATTEMPTS, INITIAL_QUANTITY, TYPE, TIME_IN_FORCE } = require("../config/constants");
...
async function transaction1(
    ...
    try {
        ...
        if (executionResponse.status === ORDER_STATUS.FILLED) {
            ...
            return transaction2(newTransactionDetail, executionResponse.executedQty);
            ...
            return Promise.allSettled([
                ...
                transaction2(newTransactionDetail, executedQty).catch(error => handleSubProcessError(error, newTransactionDetail, FUNCTION_INDEX, executedQty))
            ]);
