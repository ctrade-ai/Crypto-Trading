...
const { TRANSACTION_ATTEMPTS, ORDER_STATUS, TYPE, TIME_IN_FORCE } = require("../config/constants");
...
async function transaction2(
    ...
    try {
        ...
        if (executionResponse.status === ORDER_STATUS.FILLED) {
            ...
            return transaction3(newTransactionDetail, executionResponse.executedQty);
            ...
            const remainingAssetQty = (parseFloat(quantity) - parseFloat(executionResponse.cummulativeQuoteQty)).toString(),
                executedQty = executionResponse.executedQty;

            // Run both transactions in parallel and return their results
            return Promise.allSettled([
                deciTransaction(newTransactionDetail, remainingAssetQty, executedQty).catch(error => handleSubProcessError(error, newTransactionDetail, FUNCTION_INDEX, remainingAssetQty)),
                transaction3(newTransactionDetail, executedQty).catch(error => handleSubProcessError(error, newTransactionDetail, FUNCTION_INDEX, executedQty))
            ]);
        ...
        const processIteration = async (i) => {
            ...
            try {
                ...
                if (executionResponse.status === ORDER_STATUS.FILLED) {
                    ...
                    promises.push(transaction3(newTransactionDetail, executionResponse.executedQty).catch(error => handleSubProcessError(error, newTransactionDetail, FUNCTION_INDEX, executionResponse.executedQty)));
                } else if (parseFloat(executionResponse.executedQty)) { // Partial
                    logger.info(`${transactionDetail.processId} - Partial order placed at deci function ${FUNCTION_INDEX + 1} in ${i + 1}th part`);

                    const deciRemainingAssetQty = (parseFloat(individualQty) - parseFloat(executionResponse.cummulativeQuoteQty)).toString(),
                        deciExecutedQty = executionResponse.executedQty;

                    // Run both transactions in parallel and return their results
                    promises.push(
                        transaction3(newTransactionDetail, deciExecutedQty).catch(error => handleSubProcessError(error, newTransactionDetail, FUNCTION_INDEX, deciExecutedQty)),
